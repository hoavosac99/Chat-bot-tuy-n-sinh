...
# TODO: This value was originally imported from rasa.nlu.constants, but it was
# removed in Rasa OSS 2.0.0a3 (https://github.com/RasaHQ/rasa/pull/6466).
# Since I need to get Rasa X starting up properly, I've just copied the value
...
class DataService(DbService):
    ...
    def save_example(
        ...
        self._mark_log_as_in_training_data(new_example["hash"])
    ...
    def _mark_log_as_in_training_data(self, message_hash: Text) -> None:
        """Mark `MessageLog`s with matching text hash as included in the training data.
    ...
    def dump_nlu_data(
        ...
        for file_name in files:
            ...
            training_data = self.get_nlu_training_data_object(project, file_name)
    ...
    def delete_example(self, _id: int) -> bool:
        ...
        if result:
            ...
            self._mark_log_as_not_in_training_data(result.hash)

        return result is not None

    def _mark_log_as_not_in_training_data(self, message_hash: Text) -> None:
        """Mark `MessageLog`s with matching text hash as not included in the training
           data.
    ...
    def delete_example_by_hash(self, project_id: Text, _hash: Text) -> bool:
        ...
        self._mark_log_as_not_in_training_data(_hash)
    ...
    def replace_data(
        ...
        """Deletes all examples and adds examples from `data` in YAML, json or
    ...
    def from_request(
        ...
        return DataService(request[constants.REQUEST_DB_SESSION_KEY])
