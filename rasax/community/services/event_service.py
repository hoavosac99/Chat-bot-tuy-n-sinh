...
class EventService(DbService):
    ...
    def get_number_of_conversations(self) -> int:
        ...
        """
        return self.query(Conversation).count()
...
def read_event_broker_config(
    ...
    try:
        ...
        cli_utils.raise_warning(
            f"Could not find endpoint file at path '{rasa_x_config.endpoints_path}':\n{e}."
        )
    except KeyError:
        cli_utils.raise_warning(
            f"Could not find '{event_broker_key}' section in "
            f"endpoint config file at path '{rasa_x_config.endpoints_path}'."
        )
    except TypeError:
        cli_utils.raise_warning(
        ...
        logger.error(
            ...
            f"file at path {rasa_x_config.endpoints_path}:\n{e}"
        )
